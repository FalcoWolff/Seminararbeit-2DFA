...
\title{Two-Way Deterministic Finite Automata (2DFA)\\
\large Descriptional Advantages and Reductions to One-Way Automata}
\author{Falco Wolf}
\date{\today}
...
\begin{abstract}
Two-way deterministic finite automata (2DFA) are classical finite automata whose
reading head may move both to the left and to the right on a read-only input
tape. Although 2DFA recognize exactly the regular
languages, allowing two-way head motion can make descriptions significantly more
succinct than with ordinary one-way DFA and NFA. This seminar paper pursues two
main goals. First, we illustrate how 2DFA can describe certain regular languages
in a more natural or much more compact way, for example for suffix-dependent and
modular properties. Second, we present
constructions that simulate 2DFA by one-way automata (1NFA and 1DFA) and discuss
the resulting blow-up in the number of states and its consequences for
...
With this extend the automata can detect the left and right end of the input.
...
simulate 2DFA by one-way automata (1NFA and 1DFA) and analyze the resulting
blow-up in the number of states. In this way, we relate the descriptive
advantages of 2DFA to classical one-way models and highlight the complexity
trade-offs between them.

\section{Preliminaries}
We mostly follow the standard definitions and notation from \cite{sipser,hopcroft}.
\subsection{Words and Languages}
Fix an alphabet $\Sigma$ throughout the paper.
...
A configuration of a 1DFA/1NFA on input $w=w_1 w_2 \ldots w_n \in \Sigma^*$
is a pair $(q,i)$ with $q \in Q$ and $0 \leq i \leq n$,
...
i_{t+1} = i_t + 1
\quad\text{and}\quad
q_{t+1} \in \delta(q_t, w_{i_{t+1}}).
\]
The input $w$ is accepted by $A$ iff there exists a run
\[
(q_0,i_0),(q_1,i_1),\ldots,(q_m,i_m)
\]
with $q_m \in F$ and $i_m=n$.
...
\begin{theorem}[Subset Construction]
  For every 1NFA $A$ there exists a 1DFA $B$ such that $L(B)=L(A)$.
...
Let $w=a_0 a_1 \cdots a_{n-1} \in \Sigma^*$. A sequence
$(s_0,j_0),\ldots,(s_m,j_m)$ is a run of $A$ on $w$ if $0 \le j_m \le n$ and for
every $i$ with $0 \le i < m$ we have $0 \le j_i < n$ and there exists a pair
$(t,k) \in \rho(s_i,a_{j_i})$ such that $s_{i+1}=t$ and $j_{i+1}=j_i+k$.
The run is \emph{accepting} if $s_0 \in S_0$, $j_0=0$, $j_m=n$, and $s_m \in F$.
The automaton $A$ accepts $w$ iff it has an accepting run on $w$.
\cite{vardi_endmarkers_1990}
...
Lets now consider a 1DFA for the same language.
...
\subsection{Divisibility / Modular Properties}
\begin{example}[Length divisible by $m$]
$L_m=\{\,w\in\{a\}^* \mid |w|\equiv 0 \ (\mathrm{mod}\ m)\,\}$.
\end{example}
What to cover:
\begin{itemize}
  \item Minimal 1DFA has $m$ states (standard result).
  \item 2DFA with fewer states when splitting into primes.
  \item Special cases to reduce states further (e.g., even/odd).
\end{itemize}

\subsection{Known Succinctness Separations (optional)}
There are families of regular languages where 2DFA are exponentially smaller
than equivalent 1DFA, and cite references.
Maybe to much?

\section{Reductions: From 2DFA to One-Way Automata}
\subsection{Introduction}
At first sight, it may seem that allowing the head to move left increases the power of
finite automata beyond the regular languages. However, it is a classical result
that 2DFA still recognize exactly the regular languages; hence there is no
...
The first construction yields a one-way NFA for the complement language, and the
second uses crossing-sequence analysis to convert a 2DFA into an equivalent
one-way DFA. Our presentation follows \cite{vardi_1989_note_reduction}.
...
We follow the construction from \cite{vardi_1989_note_reduction}.
...
\subsection{Worked Example (optional)}
Show one small 2DFA and (part of) its converted 1NFA/1DFA to make the construction concrete.\newline
Maybe not possible, because of size?

\section{Conclusion}
Short wrap-up tied back to the goals.
...
\appendix
\section{Appendix}
Extra proofs, larger automata diagrams, additional examples.
