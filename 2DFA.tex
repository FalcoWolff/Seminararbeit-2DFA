...
\begin{abstract}
  Two-way automata are classical automata whose reading head may move 
  both to the left and to the right on a read-only input tape. Although 
  two-way finite automata (2FSAs) recognize exactly the regular languages, 
  allowing two-way head motion can make descriptions significantly more 
  succinct than those of ordinary one-way DFAs and NFAs.
  This seminar paper has two main objectives. First, it illustrates how 
  two-way deterministic finite automata (2DFAs) can describe certain 
  regular languages in a more compact 
  manner, for example in the case of suffix-dependent and modular 
  properties. Second, it presents constructions that simulate two-way 
  finite automata by one-way automata and discusses the resulting blow-up 
...
\begin{figure}[H]
  ...
  $L$ means move left, and $S$ means stay.}
...
\section{Why 2DFA can be More Succinct}
...
We will also talk about 1NFAs in this context.
...
with $n+2$ states, i.e. $O(n)$, for $L_n$ as follows:
\begin{itemize}
  ...
  \item Finally, it checks whether the current symbol is $1$ or $0$ and accepts
        accordingly using 1 accepting state.
\end{itemize}

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[>=stealth, on grid, auto, node distance=2.4cm]
    % states
    \node[state, initial] (q0) {$q_0$};
    \node[state, right=of q0] (q1) {$q_1$};
    \node[state, right=of q1] (q2) {$q_2$};
    \node[state, right=of q2] (q3) {$q_3$};
    \node[state, right=4.0cm of q3] (qn) {$q_n$};

    \node[state, accepting, below=2.0cm of qn] (qacc) {$q_{\mathit{acc}}$};
    ...
    % move left n positions (counting with states)
    \path[->]
      (q1) edge node {$0,1\,/\,L$} (q2)
      (q2) edge node {$0,1\,/\,L$} (q3);

    \draw[->, dotted] (q3) -- node[above] {$0,1\,/\,L$} (qn);

    % check the n-th symbol from the right (do not move)
    \path[->]
      (qn) edge node[left] {$1\,/\,S$} (qacc);

    % optional: make accept/reject halting sinks on e (or omit entirely)
    \path[->]
      (qacc) edge[loop left] node {$0,1,e\,/\,R$} (qacc);
  \end{tikzpicture}

  \caption{A 2DFA with endmarkers recognizing $L_n$.
  The automaton scans right to the endmarker $e$, moves left $n$ positions using
  states $q_1,\ldots,q_n$, and then accepts iff the current symbol is $1$.}
  \label{fig:2dfa-suffix-Ln}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[>=stealth, on grid, auto,
    ...
    \node[state, accepting]          (111) at (315:\r) {$111$};
    \node[state, accepting] (110) at (270:\r) {$110$};
    \node[state, accepting] (100) at (225:\r) {$100$};
    \node[state] (010) at (180:\r) {$010$};
    \node[state, accepting]          (101) at (135:\r) {$101$};
    ...
    % 010
    \path[->] (010) edge[bend right=12] node {$0$} (100);
    \path[->] (010) edge[bend left=12] node {$1$} (101);
  
    % 011
    \path[->] (011) edge[bend right=12] node {$0$} (110);
...
Thus, we can construct a 1DFA for $L_n$ with $2^n$ states.
...
\begin{theorem}
  ...
  \]
  for $n \geq 1$.
  Any 1DFA deciding $L_n$ requires at least $2^n$ states.
\end{theorem}
...
\begin{proof}
  ...
  because when appending $w$ the different bit is the n-th bit from the right.\\
  \\
  ...
  $\Rightarrow$ at least $2^n$ different states.\\
\end{proof}
...
For the next example we use a unary alphabet $\Sigma = \{a\}$.
...
There exists a standard 1DFA with $m$ states for $L_m$ (see, e.g., \cite{hopcroft}).
...
(Note that depending on $r$ being even or odd the automaton ends up at the left or right endmarker
and may need an additional state to move from the left to the right endmarker; we ignore this minor detail here.)
...
expressive complexity of finite automata beyond the regular languages. However, it is a classical
...
We follow Vardi's construction \cite{vardi_1989_note_reduction}.
...
\begin{lemma}[Vardi~\cite{vardi_1989_note_reduction}]
  ...
  \begin{enumerate}
    ...
    \item for every $0 \le i \le n$, every $s \in T_i$, and every transition
          $(s',k) \in \rho(s,a_i)$ with $i+k \geq 0$, we have $s' \in T_{i+k}$.
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  with $i+k \geq 0$. By definition of $T_i$, there exists a run reaching $(s,i)$.
...
The following lemma helps to prove the correctness of the construction.
...
\begin{proof}
(to simplify we assume $w$ not empty)\newline
\noindent($\Rightarrow$)
...
\medskip
\noindent
We can now prove that $T_0,\ldots,T_{n+1}$ has the properties of lemma1:
\begin{enumerate}
...
We proof that $T_0,(T_0,T_1),\ldots,(T_n,T_{n+1})$ is a accepting run for $w$ in $B$.
...
\begin{theorem}[Vardi~\cite{vardi_1989_note_reduction}]
  ...
  $B$ with $O((\exp n)^2)$ states such that $L(B)=\Sigma^* - L(A)$.
\end{theorem}
...
Let $A=(\Sigma,S,S_0,\rho,F)$ be a two-way automaton and let
$w=a_0 a_1 \cdots a_n \in \Sigma^*$.
We define an A-label as a pair $(s_1,s_2)$ where $s_1,s_2 \in S$, which represents a
run from $s_1$ to $s_2$.
...
This run is guaranteed to never visit position $i+1$.
...
We have to keep track of all possible states we can reach at position $i$.
Thus the one-way automaton must encode, in its state,
all states that can be reached in this way.
...
variants of two-way automata such as models with 
nondeterminism only at the endmarker position \cite{pighizzini_2012_two_way}, 
